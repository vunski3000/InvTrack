    ...
    <div className="min-h-screen bg-gradient-to-tr from-slate-50 via-indigo-50/30 to-purple-50/40 flex flex-col justify-center items-center p-4 relative overflow-hidden font-sans select-none">
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm z-50 p-4">
          <div className="bg-white/90 border border-white backdrop-blur-xl p-8 rounded-3xl shadow-2xl w-full max-w-sm text-center transform transition-all">
            ...
            <p className="text-slate-500 text-sm mb-6">Your administrator account has been successfully registered.</p>
            <button
              onClick={() => navigate('/login')}
              className="w-full bg-gradient-to-r from-indigo-600 to-indigo-800 hover:from-indigo-700 hover:to-indigo-900 text-white font-bold py-3 rounded-xl shadow-md transition transform active:scale-95 shadow-indigo-600/10"
            >
              Go to Login
            </button>
          </div>
      ...
      <div className="w-full max-w-md bg-white/75 border border-white/80 backdrop-blur-xl p-8 sm:p-10 rounded-3xl shadow-[0_20px_50px_-15px_rgba(99,102,241,0.12)] relative z-10 transition-all duration-300 hover:shadow-[0_25px_60px_-12px_rgba(168,85,247,0.18)]">
        <div className="mb-8 text-center">
            ...
            <p className="text-slate-500 text-sm mt-2">Provision a new administrator node.</p>
        </div>
        
        <form className="space-y-4" onSubmit={handleSignup}>
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wide">Admin Username</label>
